...
std::vector<size_t> readHeader(std::ifstream& fstream) {
    uint8_t bytes[4];
...
std::vector<size_t> readImageLabels(const std::string& filePath) {
	std::cout << "Attempting to read file: " << filePath << std::endl;
    std::ifstream fstream(filePath, std::ios::binary);
	...
	std::vector<size_t> imageLabels(dimensions[0]);
	for (size_t i = 0; i < dimensions[0]; ++i) {
		fstream.read(reinterpret_cast<char*>(&imageLabels[i]), 1);	
	}		
	return imageLabels;
}
